...
class LlamaStableTransformer:
    ...
    @staticmethod
    def circulating_to_stable_df(raw_data: List[Dict]) -> pd.DataFrame:
    ...
    @staticmethod
    def circulating_to_chain_circulating_df(raw_data: List[Dict]) -> pd.DataFrame:
        ...
        for item in raw_data:
            ...
            for chain, chain_data in chain_circulating.items():
                ...
                chain_data_obj = LlamaChainCirculatingData(
                    id=item["id"],
                    chain=chain,
    ...
    @staticmethod
    def historical_to_historical_df(raw_data: Dict) -> pd.DataFrame:
        ...
        for chain, _data in chainBalances.items():
            ...
            _df = LlamaStableTransformer.flatten_to_df(data_list)
            _df["chain"] = chain
        ...
        # selected_cols = df.columns[~df.columns.str.contains("_")]
        return df
    ...
    @staticmethod
    def flatten_to_df(data_list: List[Dict], separator: str = "_") -> pd.DataFrame:
        """
        Convert a list of potentially nested dictionaries to a flat DataFrame.

        This function is useful for processing complex API responses without having
        to manually extract each key-value pair as in historical_to_historical_df.
        ...
        """
        flattened_list = LlamaStableTransformer.flatten_nested_dict(
            data_list, separator
        )
