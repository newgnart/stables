...
def load_protocol_revenue(protocol: str, data_type: str = "historical"):
    """
    Loads protocol revenue data for a specific protocol into the DLT pipeline.
    """
    pipeline = dlt.pipeline(
        pipeline_name="ethena_defillama",
        destination="postgres",
    ...
    pipeline.run(
        protocol_revenue(protocol, data_type),
        table_name=f"protocol_revenue",
        write_disposition="merge",
...
if __name__ == "__main__":
    ...
    # Load Ethena protocol revenue data
    protocol = "ethena"
    load_protocol_revenue(protocol, data_type="historical")
